...
INSERT INTO mantenciones (carro_id, usuario_id, tipo, descripcion, realizada_en) SELECT c.id,u.id,'Preventiva','Inspección visual de bogies, revisión de fijaciones y limpieza de sensores.','2026-09-01 08:15:00' FROM carros c JOIN usuarios u ON u.correo='ana.demo@example.com' WHERE c.codigo='CARRO-101';
INSERT INTO mantenciones (carro_id, usuario_id, tipo, descripcion, realizada_en) SELECT c.id,u.id,'Correctiva','Ajuste de cierre de puerta lateral y prueba funcional de apertura y bloqueo.','2026-09-01 14:40:00' FROM carros c JOIN usuarios u ON u.correo='carlos.demo@example.com' WHERE c.codigo='CARRO-101';
INSERT INTO mantenciones (carro_id, usuario_id, tipo, descripcion, realizada_en) SELECT c.id,u.id,'Preventiva','Control de niveles, lubricación de componentes móviles y verificación de señalización.','2026-09-02 09:05:00' FROM carros c JOIN usuarios u ON u.correo='ana.demo@example.com' WHERE c.codigo='CARRO-102';
